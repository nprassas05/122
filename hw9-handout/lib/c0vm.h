...
/* Assumes sizeof(uintptr_t) >= sizeof(uint32_t) and
 *         sizeof(c0_value)  >= sizeof(uint32_t) */
static inline c0_value VAL(int32_t x) {
  return (c0_value) (uintptr_t) (uint32_t) x;
}

/* INT(p) converts generic C0 value v to int */
/* Assumes that sizeof(c0_value) >= sizeof(uint32_t) */
static inline int32_t INT(c0_value v) {
  return (int32_t) (uint32_t) (uintptr_t) v;
}
